...
The page is static HTML with a page-specific stylesheet, `assets/css/research.css`. All visuals and tables work when opening the HTML locally; no fetching, rendering library, or JavaScript is needed. Keep internal links explicit (`index.html`). Legacy section links continue to resolve. Add each new paper once under its primary theme, with one appropriately sourced visual and a short contribution sentence. Do not add paper counts to the topic navigation. Remove the `Preprint` badge when a paper is accepted or published. Bibliographic metadata is retained from the existing website rather than inferred from arXiv submission dates.
